...
export default function WebDevServices() {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      {/* Hero Section */}
      <section className="relative h-[600px] flex items-center justify-center text-center bg-red-700">
...
<section id="services" className="container mx-auto my-16 p-10">
  ...
  <ul className="text-gray-700 text-center grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
    {services.map((service, i) => (
      <li key={i} className="p-6 bg-white rounded-lg shadow-md hover:shadow-xl transition-shadow duration-300 ease-in-out hover:bg-blue-50">
        {service}
      </li>
    ))}
  </ul>
...
<section className="bg-gray-200 py-16">
  ...
  <div className="flex flex-wrap justify-center items-center gap-4">
    {techStacks.map((tech, i) => (
      <span key={i} className={`p-2 rounded-full text-white shadow-md ${getColor(i)}`}>
        {tech}
      </span>
    ))}
  </div>
