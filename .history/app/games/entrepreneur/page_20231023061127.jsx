...
const Quiz = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-100 py-12 pt-20">
      <h1 className="text-4xl font-bold text-white mb-8 pt-12 md:pt-24">Entrepreneur Quiz</h1>
      {!quizStarted ? (
        <div className="max-w-lg mx-auto bg-white p-8 rounded-lg shadow text-center">
          ...
          />
          <button 
            className="text-white bg-purple-800 rounded px-4 py-2"
            onClick={startQuiz}
          >
            Start Quiz
          </button>
    ...
    <div className="flex flex-col space-y-4 text-lg"> {/* Added flex and flex-col classes */}
        {questions[currentQuestion].answers.map((answer, index) => (
            <button
                onClick={() => handleAnswerOptionClick(answer.value)}
                key={index}
                className="mb-2 text-white bg-purple-900 rounded px-4 py-2"
            >
                {answer.text}
            </button>
        ))}
