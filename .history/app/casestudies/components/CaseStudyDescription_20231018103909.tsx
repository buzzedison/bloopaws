...
const CaseStudyDescription: React.FC<Props> = ({ title, description, links, imageUrl, challenge, solution, results }) => {
  return (
    <motion.div className="p-10 bg-gray-100 rounded-xl shadow-2xl space-y-10" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }}>
        ...
        <div className="mt-6 flex flex-wrap justify-center gap-6">
          {links.map((link, index) => (
            <Link key={index} href={link.url}>
              <motion.a className="text-xl text-blue-600 hover:text-blue-700 transition-colors duration-300 cursor-pointer" whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }}>
                {link.text}
              </motion.a>
            </Link>
          ))}
