...
export default function HeroPost({ post }: HeroPostProps) {
  return (
    <div className="w-full h-full md:h-screen md:w-screen overflow-hidden">
      ...
      <div className="absolute inset-0 bg-black bg-opacity-50 flex flex-col justify-center items-center p-8 space-y-8 transition-all ease-in-out duration-500 transform translate-y-4 hover:translate-y-0">
        <Link href={post.slug.current} className="text-5xl font-bold text-white hover:text-blue-300 transition ease-in-out duration-300 text-center">
          {post.title}
        </Link>
        <p className="text-xl text-white max-w-2xl text-center">
          {post.excerpt} {/* Displaying the excerpt */}
        </p>
        <Link href={post.slug.current} className="flex items-center text-xl text-white hover:text-blue-300 transition ease-in-out duration-300">
          Read More 
