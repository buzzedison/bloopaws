          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            {sections.map((section) => (
              ...
              <div key={section.title} className="bg-white shadow-lg rounded-lg overflow-hidden flex flex-col lg:pr-12 w-3/4 h-3/4 mx-auto">
                // Use object-fit property to scale the images to fit the container
                <Image className="w-full" src={section.image} alt={section.alt} width={450} height={150} objectFit="cover" />
                <div className="p-4 flex flex-col justify-between flex-grow">
                  <div>
                    // Use a lighter font weight and a smaller font size for the titles
                    <h2 className="text-xl font-medium mb-2 pt-4 pb-4 ">{section.title}</h2>
                    // Use a darker text color and a smaller font size for the descriptions
                    <p className="text-sm text-gray-800 mb-4">{section.description}</p>
                  </div>
                  <Link href={section.button.link} passHref>
                    // Use a smaller font size and padding for the buttons
                    <button className="inline-block bg-red-600 text-white px-4 py-2 rounded-lg hover:bg-red-700 self-start text-sm">{section.button.text}</button>
                  </Link>
                </div>
              </div>
            ))}
