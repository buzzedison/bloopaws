...
async function getPost() {
  ...
  export default async function HeroNew() {
    ...
    return (
      <div className="relative h-[500px] md:h-[600px] bg-cover bg-center" style={{ backgroundImage: "url('/path/to/your/background/image.jpg')" }}>
        <div className="absolute inset-0 bg-gradient-to-b from-transparent to-black opacity-50"></div>
        <div className="container mx-auto px-4 h-full flex items-center justify-center text-center">
          <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold mb-8 text-white z-10">Blog Posts</h1>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {blog.map((post) => (
              ...
              >
                ...
                />
                 <Link href={`/blog/${post.uri}`}> Learn more</Link>
