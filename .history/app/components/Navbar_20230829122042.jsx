...
const Navbar = () => {
  ...
  return (
    <nav className="fixed top-0 left-0 w-full z-50 flex items-center justify-between bg-opacity-80 backdrop-blur-md px-4 py-2 text-white">
      ...
      {/* Mobile Menu */}
      {isMobileMenuOpen && (
        <div className="fixed top-0 left-0 w-full h-full bg-black bg-opacity-50  justify-center items-center z-9999">
          <div className="bg-transparent rounded-lg w-3/4 h-3/4 overflow-y-auto">
            <button onClick={() => setIsMobileMenuOpen(false)} className="absolute top-4 right-4 text-black z-10000">
              Close
            </button>
            <ul className="flex flex-col space-y-4 p-8">
              ...
              />
              <li><Link className="flex" href="/">Resources</Link></li>
              <li><Link href="/">Brands</Link></li>
              <li><Link href="/">About</Link></li>
              <li><Link href="/">Insight</Link></li>
              <li><Link href="/" className="bg-red-600 text-white px-6 py-2 rounded-full hover:bg-red-500 transition duration-300">Sign Up</Link></li>
