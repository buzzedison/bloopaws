  ...
  export default async function BlogGrid () {
    ...
    return (
        <div className="container mx-auto px-4 py-12">
            <h1 className="text-3xl font-bold mb-8 text-centre">Blog Posts</h1>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {blog.slice(0, 3).map((post) => (
                    <div
                        key={post.id}
                        className="bg-white rounded-lg shadow-md p-0 transition-all duration-200 transform hover:scale-105 hover:bg-gray-100 flex flex-col"
                    >
                        ...
                        <h2 className="text-2xl font-semibold mb-4 flex-grow">{post.title}</h2>
                        <div
                            className="text-gray-600"
                            dangerouslySetInnerHTML={{ __html: post.excerpt.substring(0, 100) }}
                        />
                        <Link href={`/blog/${post.uri}`}className="text-blue-500 hover:text-blue-700">
                                Learn more
