...
const criteriaData: CriteriaProps[] = [
  {
    ...
    imagePath: '/images/fund/team.png', // Replace with the actual path to your image file
  },
  {
    title: 'Market Demand',
...
const InvestmentCriteria: FunctionComponent = () => {
  return (
    <section className="container mx-auto px-6 py-16">
      <h2 className="text-3xl font-bold text-center margin-bottom-12">Investment Criteria</h2>
      <div className="grid grid-cols-1 small:grid-cols-2 large:grid-cols-4 gap-10">
        {criteriaData.map((criterion, index) => (
          <div key={index} className="background-white padding-6 rounded-lg shadow-lg hover:shadow-2xl transition-shadow duration-300">
            <div className="flex justify-center items-center">
              <Image src={criterion.imagePath} width={50} height={50} alt={criterion.title} layout="fixed" />
            </div>
            <h3 className="text-lg font-semibold text-center margin-top-4">{criterion.title}</h3>
            <p className="text-center margin-top-2">{criterion.description}</p>
          </div>
        ))}
      </div>
    </section>
